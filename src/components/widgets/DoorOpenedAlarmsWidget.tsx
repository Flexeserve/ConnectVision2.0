  ...
  return (
    ...
    >
      {size === "large" ? (
        <div className="flex min-h-0 flex-1 flex-col gap-2">
          <div className="flex items-center justify-between">
            <div className="text-2xl font-bold tabular-nums text-ink">
              {latest}
              <span className="ml-1 text-sm font-semibold text-ink-muted">°C</span>
            </div>
            <select
              value={days}
        ...
        <>
          <WidgetMetric value={latest} unit="°C" />
          <div className="text-center text-xs text-ink-muted">Latest reading</div>
        </>
      )}
    </WidgetShell>
  );
